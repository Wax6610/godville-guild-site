...
import {Table, Segment, Pagination, Form, Progress} from "semantic-ui-react";
...
export default class MainTable extends PureComponent {
    ...
    getActivePageData = () => {
        const {pagination: {perPage, activePage}, data: {filteredData}} = this.state;
    ...
    handleSort = (clickedColumn) => () => {
        ...
        if (column !== clickedColumn) {
            this.setState({
                ...
                data: {...this.state.data, filteredData: _.orderBy(filteredData, [clickedColumn], ['desc']),}
            })
            return
        }
        this.setState({
    ...
    getMaxcellValues = (rawData) => {
        ...
        rawData.map(row => {
            if (row.bricks_cnt > maxCellValues.bricks_cnt) maxCellValues.bricks_cnt = row.bricks_cnt
            if (row.wood_cnt > maxCellValues.wood_cnt) maxCellValues.wood_cnt = row.wood_cnt
            if (row.savings > maxCellValues.savings) maxCellValues.savings = row.savings
            if (row.arena_won > maxCellValues.arena_won) maxCellValues.arena_won = row.arena_won
            if (row.arena_lost > maxCellValues.arena_lost) maxCellValues.arena_lost = row.arena_lost
            if (row.ark_m > maxCellValues.ark_m) maxCellValues.ark_m = row.ark_m
            if (row.ark_f > maxCellValues.ark_f) maxCellValues.ark_f = row.ark_f
    ...
    tableInit = rawData => {
        ...
        this.setState({
            column: null,
            direction: null,
            pagination: {...this.state.pagination, activePage: 1, totalPages: Math.ceil(rawData.length / perPage)},
            data: {
                ...this.state.data,
                rawData: rawData,
            },
    ...
    /* Фильтрация данных*/
    filterData = () => {
        const {data: {rawData}, pagination: {perPage}, filter, column, direction} = this.state;

        let filteredData = rawData
            .filter(row => filter.clan_position === "" || row.clan_position === filter.clan_position)
            .filter(
                row =>
                    filter.name === "" || row.name.toLowerCase().indexOf(filter.name) > -1
            )
        ...
        this.setState(
            {
                ...
                pagination: {...this.state.pagination, totalPages: Math.ceil(filteredData.length / perPage)}
            }
        );
...
const MainTableHeader = (props) => {
    ...
    const {column, direction, handleSort, handleInputFilterChange, handleSelectFilterChange} = props
    const headers = [
        {name: "name", title: "Бог", filter: 'input'},
        {name: "level", title: "Уровень"},
        {name: "clan_position", title: "Звание", filter: 'select', filterOptions: filterClanPosition},
        ...
        <Table.Row>
            {headers.map(header => (
                <Table.Cell key={header.name}>
                    {(header.filter === 'input') ? (<Form.Input name={header.name}
                                                                onChange={handleInputFilterChange}></Form.Input>) : ''}
                    {(header.filter === 'select') ? (
                        <Form.Select options={header.filterOptions} name={header.name}
                                     onChange={handleSelectFilterChange}></Form.Select>) : ''}
                </Table.Cell>
            ))}
        </Table.Row>
    </Table.Header>)
